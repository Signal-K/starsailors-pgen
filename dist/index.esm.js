...
import React__default, { useRef, useMemo, useEffect, forwardRef, createElement, useLayoutEffect, useState, useCallback } from 'react';
...
function Planet({ config }) {
    ...
    const shaderMaterial = useMemo(() => {
        ...
        const material = new THREE.ShaderMaterial({
            ...
            uniforms: {
                ...
                visibleTerrains: {
                    value: new THREE.Vector4(safeConfig.visibleTerrains?.ocean ? 1.0 : 0.0, safeConfig.visibleTerrains?.beach ? 1.0 : 0.0, safeConfig.visibleTerrains?.lowland ? 1.0 : 0.0, safeConfig.visibleTerrains?.midland ? 1.0 : 0.0),
                },
                visibleTerrains2: {
                    value: new THREE.Vector4(safeConfig.visibleTerrains?.highland ? 1.0 : 0.0, safeConfig.visibleTerrains?.mountain ? 1.0 : 0.0, safeConfig.visibleTerrains?.snow ? 1.0 : 0.0, 0.0),
                },
                // Colors
    ...
    return (jsxs("group", { children: [jsxs("mesh", { ref: meshRef, scale: safeConfig.radius, children: [jsx("sphereGeometry", { args: [1, 128, 128] }), jsx("primitive", { object: shaderMaterial, attach: "material" })] }), jsxs("mesh", { ref: atmosphereRef, scale: safeConfig.radius * (safeConfig.type === "gaseous" ? 1.15 : 1.05), children: [jsx("sphereGeometry", { args: [1, 64, 64] }), jsx("meshBasicMaterial", { color: safeConfig.colors.atmosphere, transparent: true, opacity: safeConfig.type === "gaseous" ? 0.7 : 0.3, side: THREE.BackSide })] })] }));
}
...
SliderBubbleInput.displayName = BUBBLE_INPUT_NAME$3;
...
function ColorPicker({ color, onChange, label }) {
    ...
    return (jsxs("div", { className: "flex flex-col space-y-1.5", children: [label && jsx(Label$1, { className: "text-xs", children: label }), jsxs(Popover, { open: isOpen, onOpenChange: setIsOpen, children: [jsx(PopoverTrigger, { asChild: true, children: jsxs("div", { className: "flex h-8 cursor-pointer", children: [jsx("div", { className: "w-8 h-8 rounded-l border border-r-0 border-gray-600", style: { backgroundColor: localColor } }), jsx(Input, { type: "text", value: localColor, onChange: handleInputChange, className: "rounded-l-none w-24 h-8 font-mono text-xs", onClick: (e) => e.stopPropagation() })] }) }), jsx(PopoverContent, { className: "w-auto p-3", side: "right", children: jsx(Z, { color: localColor, onChange: handleColorChange }) })] })] }));
}
...
function SettingsPanel({ planetConfig, onChange }) {
    ...
    const liquidType = getLiquidType(planetConfig.temperature);
    return (jsxs("div", { className: "space-y-6 bg-slate-900 text-slate-100", children: [jsx("div", { className: "flex justify-between items-center", children: jsxs(Button, { variant: "outline", size: "sm", onClick: () => setShowImportExport(!showImportExport), className: "flex items-center gap-2 bg-slate-800 text-slate-200 border-slate-700 hover:bg-slate-700", children: [showImportExport ? "Hide" : "Show", " Import/Export", showImportExport ? jsx(Download, { className: "h-4 w-4" }) : jsx(Upload, { className: "h-4 w-4" })] }) }), showImportExport && (jsxs("div", { className: "p-4 bg-slate-800 rounded-md space-y-4 border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium text-slate-200", children: "Import/Export Configuration" }), jsxs("div", { className: "flex gap-2", children: [jsx(Button, { variant: "secondary", size: "sm", onClick: () => {
                                    const event = new CustomEvent("open-import-dialog");
                                    window.dispatchEvent(event);
                                }, className: "flex-1 bg-slate-700 text-slate-200 hover:bg-slate-600", children: "Import" }), jsx(Button, { variant: "secondary", size: "sm", onClick: () => {
                                    const event = new CustomEvent("open-export-dialog");
                                    window.dispatchEvent(event);
                                }, className: "flex-1 bg-slate-700 text-slate-200 hover:bg-slate-600", children: "Export" })] })] })), jsxs(Tabs, { defaultValue: "basic", className: "w-full", children: [jsxs(TabsList, { className: "w-full grid grid-cols-3 mb-4 bg-slate-800", children: [jsx(TabsTrigger, { value: "basic", className: "data-[state=active]:bg-slate-700", children: "Basic" }), jsx(TabsTrigger, { value: "terrain", className: "data-[state=active]:bg-slate-700", children: "Terrain" }), jsx(TabsTrigger, { value: "colors", className: "data-[state=active]:bg-slate-700", children: "Colors" })] }), jsx(TabsContent, { value: "basic", className: "space-y-6", children: jsxs("div", { className: "space-y-4", children: [jsxs("div", { className: "bg-slate-800 p-4 rounded-md border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium mb-3 text-slate-200", children: "Planet Type" }), jsxs("div", { className: "flex gap-2", children: [jsx(Button, { variant: planetConfig.type === "terrestrial" ? "default" : "outline", size: "sm", onClick: () => onChange({ type: "terrestrial" }), className: "flex-1", children: "Terrestrial" }), jsx(Button, { variant: planetConfig.type === "gaseous" ? "default" : "outline", size: "sm", onClick: () => onChange({ type: "gaseous" }), className: "flex-1", children: "Gaseous" })] }), (planetConfig.mass > 7.5 || planetConfig.radius > 2.5) && planetConfig.type === "gaseous" && (jsx("p", { className: "text-xs text-amber-400 mt-2", children: "Note: High mass or radius has automatically set this to a gas giant" })), planetConfig.mass <= 7.5 && planetConfig.radius <= 2.5 && planetConfig.type === "terrestrial" && (jsx("p", { className: "text-xs text-slate-400 mt-2", children: "Note: Values are in Earth radii/mass units" }))] }), jsxs("div", { className: "bg-slate-800 p-4 rounded-md space-y-4 border border-slate-700", children: [jsxs("div", { className: "flex justify-between items-center", children: [jsxs("h3", { className: "text-sm font-medium text-slate-200", children: ["Seed: ", planetConfig.seed] }), jsx(Button, { variant: "outline", size: "icon", onClick: regenerateSeed, className: "bg-slate-700 border-slate-600", children: jsx(RefreshCw, { className: "h-4 w-4" }) })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "radius", className: "text-slate-200", children: "Radius (Earth radii)" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.radius.toFixed(1) })] }), jsx(Slider, { id: "radius", min: 0.5, max: 5, step: 0.1, value: [planetConfig.radius], onValueChange: (value) => onChange({ radius: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "temperature", className: "text-slate-200", children: "Temperature" }), jsxs("span", { className: "text-sm text-slate-400", children: [planetConfig.temperature.toFixed(0), "K (", (planetConfig.temperature - 273.15).toFixed(0), "\u00B0C)"] })] }), jsx(Slider, { id: "temperature", min: 50, max: 700, step: 1, value: [planetConfig.temperature], onValueChange: (value) => onChange({ temperature: value[0] }), className: "[&_[role=slider]]:bg-slate-200" }), jsxs("div", { className: "text-xs text-slate-400", children: ["Liquid Solvent: ", liquidType.name] })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "mass", className: "text-slate-200", children: "Mass (Earth masses)" }), jsxs("span", { className: "text-sm text-slate-400", children: [planetConfig.mass.toFixed(1), " M\u2295"] })] }), jsx(Slider, { id: "mass", min: 0.1, max: 10, step: 0.1, value: [planetConfig.mass], onValueChange: (value) => onChange({ mass: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsx("div", { className: "pt-2 border-t border-slate-700", children: jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { className: "text-slate-200", children: "Density" }), jsxs("span", { className: "text-sm text-slate-400", children: [(planetConfig.mass / ((planetConfig.radius ** 3 * Math.PI * 4) / 3)).toFixed(2), " g/cm\u00B3"] })] }) })] })] }) }), jsxs(TabsContent, { value: "terrain", className: "space-y-6", children: [planetConfig.type === "terrestrial" && (jsxs("div", { className: "bg-slate-800 p-4 rounded-md space-y-4 border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium text-slate-200", children: "Terrain Properties" }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "terrainRoughness", className: "text-slate-200", children: "Terrain Roughness" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.terrainRoughness.toFixed(2) })] }), jsx(Slider, { id: "terrainRoughness", min: 0.1, max: 1, step: 0.01, value: [planetConfig.terrainRoughness], onValueChange: (value) => onChange({ terrainRoughness: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "liquidHeight", className: "text-slate-200", children: "Ocean Level" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.liquidHeight.toFixed(2) })] }), jsx(Slider, { id: "liquidHeight", min: 0.3, max: 0.8, step: 0.01, value: [planetConfig.liquidHeight], onValueChange: (value) => onChange({ liquidHeight: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "continentSize", className: "text-slate-200", children: "Continent Size" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.continentSize.toFixed(2) })] }), jsx(Slider, { id: "continentSize", min: 0.1, max: 1, step: 0.01, value: [planetConfig.continentSize], onValueChange: (value) => onChange({ continentSize: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "continentCount", className: "text-slate-200", children: "Continent Count" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.continentCount.toFixed(0) })] }), jsx(Slider, { id: "continentCount", min: 1, max: 10, step: 1, value: [planetConfig.continentCount], onValueChange: (value) => onChange({ continentCount: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "noiseScale", className: "text-slate-200", children: "Detail Level" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.noiseScale.toFixed(2) })] }), jsx(Slider, { id: "noiseScale", min: 0.5, max: 2, step: 0.1, value: [planetConfig.noiseScale], onValueChange: (value) => onChange({ noiseScale: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "volcanicActivity", className: "text-slate-200", children: "Volcanic Activity" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.volcanicActivity.toFixed(2) })] }), jsx(Slider, { id: "volcanicActivity", min: 0, max: 1, step: 0.01, value: [planetConfig.volcanicActivity], onValueChange: (value) => onChange({ volcanicActivity: value[0] }), className: "[&_[role=slider]]:bg-slate-200" }), jsx("div", { className: "text-xs text-slate-400 italic", children: "Note: Volcanic activity is currently a static value" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "biomass", className: "text-slate-200", children: "Biomass" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.biomass.toFixed(2) })] }), jsx(Slider, { id: "biomass", min: 0, max: 1, step: 0.01, value: [planetConfig.biomass], onValueChange: (value) => onChange({ biomass: value[0] }), className: "[&_[role=slider]]:bg-slate-200" }), jsx("div", { className: "text-xs text-slate-400 italic", children: "Note: Biomass is currently a static value" })] })] })), jsxs("div", { className: "bg-slate-800 p-4 rounded-md space-y-4 border border-slate-700", children: [jsxs("div", { className: "flex justify-between items-center", children: [jsx("h3", { className: "text-sm font-medium text-slate-200", children: "Debug Mode" }), jsx(Switch, { checked: planetConfig.debugMode || false, onCheckedChange: (checked) => onChange({ debugMode: checked }) })] }), planetConfig.debugMode && (jsxs("div", { className: "pt-2 border-t border-slate-700 space-y-2", children: [jsx("p", { className: "text-xs text-slate-400", children: "Show/hide specific terrain types:" }), jsxs("div", { className: "grid grid-cols-2 gap-2", children: [jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-ocean", checked: planetConfig.visibleTerrains.ocean, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        ocean: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-ocean", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.ocean } }), jsx("span", { children: "Ocean" })] }) })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-beach", checked: planetConfig.visibleTerrains.beach, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        beach: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-beach", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.beach } }), jsx("span", { children: "Beach" })] }) })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-lowland", checked: planetConfig.visibleTerrains.lowland, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        lowland: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-lowland", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.lowland } }), jsx("span", { children: "Lowland" })] }) })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-midland", checked: planetConfig.visibleTerrains.midland, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        midland: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-midland", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.midland } }), jsx("span", { children: "Midland" })] }) })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-highland", checked: planetConfig.visibleTerrains.highland, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        highland: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-highland", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.highland } }), jsx("span", { children: "Highland" })] }) })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-mountain", checked: planetConfig.visibleTerrains.mountain, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        mountain: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-mountain", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.mountain } }), jsx("span", { children: "Mountain" })] }) })] }), jsxs("div", { className: "flex items-center space-x-2", children: [jsx(Checkbox, { id: "show-snow", checked: planetConfig.visibleTerrains.snow, onCheckedChange: (checked) => onChange({
                                                                    visibleTerrains: {
                                                                        ...planetConfig.visibleTerrains,
                                                                        snow: !!checked,
                                                                    },
                                                                }) }), jsx(Label$1, { htmlFor: "show-snow", className: "text-xs cursor-pointer", children: jsxs("div", { className: "flex items-center gap-2", children: [jsx("div", { className: "w-3 h-3 rounded-full", style: { backgroundColor: planetConfig.colors.snow } }), jsx("span", { children: "Snow" })] }) })] })] })] }))] })] }), jsx(TabsContent, { value: "colors", className: "space-y-6", children: jsxs("div", { className: "bg-slate-800 p-4 rounded-md space-y-4 border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium text-slate-200", children: "Planet Colors" }), jsxs("div", { className: "grid grid-cols-2 gap-4", children: [jsx(ColorPicker, { label: "Atmosphere", color: planetConfig.colors.atmosphere, onChange: (color) => onChange({ colors: { ...planetConfig.colors, atmosphere: color } }) }), jsx(ColorPicker, { label: `${liquidType.name}`, color: planetConfig.colors.ocean, onChange: (color) => onChange({ colors: { ...planetConfig.colors, ocean: color } }) }), jsx(ColorPicker, { label: `${liquidType.name} Pattern`, color: planetConfig.colors.oceanPattern, onChange: (color) => onChange({ colors: { ...planetConfig.colors, oceanPattern: color } }) }), jsx(ColorPicker, { label: "Beach", color: planetConfig.colors.beach, onChange: (color) => onChange({ colors: { ...planetConfig.colors, beach: color } }) }), jsx(ColorPicker, { label: "Lowland", color: planetConfig.colors.lowland, onChange: (color) => onChange({ colors: { ...planetConfig.colors, lowland: color } }) }), jsx(ColorPicker, { label: "Midland", color: planetConfig.colors.midland, onChange: (color) => onChange({ colors: { ...planetConfig.colors, midland: color } }) }), jsx(ColorPicker, { label: "Highland", color: planetConfig.colors.highland, onChange: (color) => onChange({ colors: { ...planetConfig.colors, highland: color } }) }), jsx(ColorPicker, { label: "Mountain", color: planetConfig.colors.mountain, onChange: (color) => onChange({ colors: { ...planetConfig.colors, mountain: color } }) }), jsx(ColorPicker, { label: "Snow", color: planetConfig.colors.snow, onChange: (color) => onChange({ colors: { ...planetConfig.colors, snow: color } }) })] })] }) })] })] }));
}

function PlanetViewer({ planetConfig, onConfigChange }) {
    const [showSettings, setShowSettings] = useState(false);
    return (jsxs("div", { className: "w-full h-full bg-black relative", children: [jsxs(Canvas, { camera: { position: [0, 0, 10], fov: 45 }, children: [jsx("ambientLight", { intensity: 0.3 }), jsx("pointLight", { position: [10, 10, 10], intensity: 1.5 }), jsx(Planet, { config: planetConfig }), jsx(OrbitControls, { enablePan: true, enableZoom: true, enableRotate: true }), jsx(Stars, { radius: 100, depth: 50, count: 5000, factor: 4, saturation: 0, fade: true, speed: 1 })] }), jsx("button", { onClick: () => setShowSettings(!showSettings), className: "absolute top-4 right-4 bg-slate-800 p-2 rounded-full text-white hover:bg-slate-700 transition-colors z-10 border border-slate-700", "aria-label": "Toggle settings", children: jsx(Cog, { className: "w-6 h-6" }) }), showSettings && (jsxs("div", { className: "absolute top-0 right-0 w-full md:w-96 h-full bg-slate-900/95 text-white overflow-y-auto p-4 backdrop-blur-sm z-10 transition-all border-l border-slate-700", children: [jsxs("div", { className: "flex justify-between items-center mb-4", children: [jsx("h2", { className: "text-xl font-bold text-slate-100", children: "Planet Settings" }), jsx("button", { onClick: () => setShowSettings(false), className: "text-slate-300 hover:text-slate-100", "aria-label": "Close settings", children: jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("line", { x1: "18", y1: "6", x2: "6", y2: "18" }), jsx("line", { x1: "6", y1: "6", x2: "18", y2: "18" })] }) })] }), jsx(SettingsPanel, { planetConfig: planetConfig, onChange: onConfigChange })] }))] }));
}
...
const SelectTrigger = React.forwardRef(({ className, children, ...props }, ref) => (jsxs(Trigger, { ref: ref, className: cn("flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background data-[placeholder]:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1", className), ...props, children: [children, jsx(Icon, { asChild: true, children: jsx(ChevronDown, { className: "h-4 w-4 opacity-50" }) })] })));
...
const SelectLabel = React.forwardRef(({ className, ...props }, ref) => (jsx(Label, { ref: ref, className: cn("py-1.5 pl-8 pr-2 text-sm font-semibold", className), ...props })));
SelectLabel.displayName = Label.displayName;
const SelectItem = React.forwardRef(({ className, children, ...props }, ref) => (jsxs(Item, { ref: ref, className: cn("relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none focus:bg-accent focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50", className), ...props, children: [jsx("span", { className: "absolute left-2 flex h-3.5 w-3.5 items-center justify-center", children: jsx(ItemIndicator, { children: jsx(Check, { className: "h-4 w-4" }) }) }), jsx(ItemText, { children: children })] })));
SelectItem.displayName = Item.displayName;
const SelectSeparator = React.forwardRef(({ className, ...props }, ref) => (jsx(Separator, { ref: ref, className: cn("-mx-1 my-1 h-px bg-muted", className), ...props })));
...
function SimplePlanetSettings({ planetConfig, onChange }) {
    ...
    return (jsxs("div", { className: "space-y-6 bg-slate-900 text-slate-100", children: [jsx("div", { className: "flex justify-between items-center", children: jsxs(Button, { variant: "outline", size: "sm", onClick: () => setShowImportExport(!showImportExport), className: "flex items-center gap-2 bg-slate-800 text-slate-200 border-slate-700 hover:bg-slate-700", children: [showImportExport ? "Hide" : "Show", " Import/Export", showImportExport ? jsx(Download, { className: "h-4 w-4" }) : jsx(Upload, { className: "h-4 w-4" })] }) }), showImportExport && (jsxs("div", { className: "p-4 bg-slate-800 rounded-md space-y-4 border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium text-slate-200", children: "Import/Export Configuration" }), jsxs("div", { className: "flex gap-2", children: [jsx(Button, { variant: "secondary", size: "sm", onClick: () => {
                                    ...
                                    window.dispatchEvent(event);
                                }, className: "flex-1 bg-slate-700 text-slate-200 hover:bg-slate-600", children: "Export" })] })] })), jsxs("div", { className: "bg-slate-800 p-4 rounded-md border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium mb-3 text-slate-200", children: "Presets" }), jsxs(Select, { onValueChange: loadPreset, children: [jsx(SelectTrigger, { className: "bg-slate-700 border-slate-600", children: jsx(SelectValue, { placeholder: "Load a preset..." }) }), jsxs(SelectContent, { children: [jsx(SelectItem, { value: "earth", children: "Earth-like" }), jsx(SelectItem, { value: "mars", children: "Mars-like" }), jsx(SelectItem, { value: "venus", children: "Venus-like" }), jsx(SelectItem, { value: "jupiter", children: "Jupiter-like" }), jsx(SelectItem, { value: "neptune", children: "Neptune-like" })] })] })] }), jsxs("div", { className: "bg-slate-800 p-4 rounded-md border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium mb-3 text-slate-200", children: "Planet Type" }), jsxs("div", { className: "flex gap-2", children: [jsx(Button, { variant: planetConfig.type === "terrestrial" ? "default" : "outline", size: "sm", onClick: () => onChange({ type: "terrestrial" }), className: "flex-1", children: "Terrestrial" }), jsx(Button, { variant: planetConfig.type === "gaseous" ? "default" : "outline", size: "sm", onClick: () => onChange({ type: "gaseous" }), className: "flex-1", children: "Gaseous" })] }), (planetConfig.mass > 7.5 || planetConfig.radius > 2.5) && (jsx("p", { className: "text-xs text-amber-400 mt-2", children: "Note: High mass or radius automatically sets this to gaseous" }))] }), jsxs("div", { className: "bg-slate-800 p-4 rounded-md space-y-4 border border-slate-700", children: [jsxs("div", { className: "flex justify-between items-center", children: [jsxs("h3", { className: "text-sm font-medium text-slate-200", children: ["Seed: ", planetConfig.seed] }), jsx(Button, { variant: "outline", size: "icon", onClick: regenerateSeed, className: "bg-slate-700 border-slate-600", children: jsx(RefreshCw, { className: "h-4 w-4" }) })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "radius", className: "text-slate-200", children: "Radius (Earth radii)" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.radius.toFixed(1) })] }), jsx(Slider, { id: "radius", min: 0.3, max: 5, step: 0.1, value: [planetConfig.radius], onValueChange: (value) => onChange({ radius: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "mass", className: "text-slate-200", children: "Mass (Earth masses)" }), jsxs("span", { className: "text-sm text-slate-400", children: [planetConfig.mass.toFixed(1), " M\u2295"] })] }), jsx(Slider, { id: "mass", min: 0.1, max: 10, step: 0.1, value: [planetConfig.mass], onValueChange: (value) => onChange({ mass: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "temperature", className: "text-slate-200", children: "Temperature" }), jsxs("span", { className: "text-sm text-slate-400", children: [planetConfig.temperature.toFixed(0), "K (", (planetConfig.temperature - 273.15).toFixed(0), "\u00B0C)"] })] }), jsx(Slider, { id: "temperature", min: 50, max: 700, step: 5, value: [planetConfig.temperature], onValueChange: (value) => onChange({ temperature: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsxs("div", { className: "space-y-2", children: [jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { htmlFor: "biomass", className: "text-slate-200", children: "Biomass" }), jsx("span", { className: "text-sm text-slate-400", children: planetConfig.biomass.toFixed(2) })] }), jsx(Slider, { id: "biomass", min: 0, max: 1, step: 0.1, value: [planetConfig.biomass], onValueChange: (value) => onChange({ biomass: value[0] }), className: "[&_[role=slider]]:bg-slate-200" })] }), jsx("div", { className: "pt-2 border-t border-slate-700", children: jsxs("div", { className: "flex justify-between", children: [jsx(Label$1, { className: "text-slate-200", children: "Density" }), jsxs("span", { className: "text-sm text-slate-400", children: [density.toFixed(2), " g/cm\u00B3"] })] }) })] }), jsxs("div", { className: "bg-slate-800 p-4 rounded-md space-y-4 border border-slate-700", children: [jsx("h3", { className: "text-sm font-medium text-slate-200", children: "Colors" }), jsxs("div", { className: "space-y-4", children: [jsx(ColorPicker, { label: planetConfig.type === "gaseous" ? "Atmosphere" : "Ocean/Water", color: simplifiedColors.primary, onChange: (color) => handleColorChange("primary", color) }), jsx(ColorPicker, { label: planetConfig.type === "gaseous" ? "Bands" : "Land/Vegetation", color: simplifiedColors.secondary, onChange: (color) => handleColorChange("secondary", color) }), jsx(ColorPicker, { label: planetConfig.type === "gaseous" ? "Storms" : "Mountains/Highlands", color: simplifiedColors.accent, onChange: (color) => handleColorChange("accent", color) })] })] })] }));
}

function SimplePlanetViewer({ planetConfig, onConfigChange }) {
    const [showSettings, setShowSettings] = useState(false);
    return (jsxs("div", { className: "w-full h-full bg-black relative", children: [jsxs(Canvas, { camera: { position: [0, 0, 5], fov: 45 }, children: [jsx("ambientLight", { intensity: 0.3 }), jsx("pointLight", { position: [10, 10, 10], intensity: 1.5 }), jsx(SimplePlanetMesh, { config: planetConfig }), jsx(OrbitControls, { enablePan: true, enableZoom: true, enableRotate: true, minDistance: 2, maxDistance: 10 }), jsx(Stars, { radius: 100, depth: 50, count: 5000, factor: 4, saturation: 0, fade: true, speed: 1 })] }), jsx("button", { onClick: () => setShowSettings(!showSettings), className: "absolute top-4 right-4 bg-slate-800 p-2 rounded-full text-white hover:bg-slate-700 transition-colors z-10 border border-slate-700", "aria-label": "Toggle settings", children: jsx(Cog, { className: "w-6 h-6" }) }), showSettings && (jsxs("div", { className: "absolute top-0 right-0 w-full md:w-96 h-full bg-slate-900/95 text-white overflow-y-auto p-4 backdrop-blur-sm z-10 transition-all border-l border-slate-700", children: [jsxs("div", { className: "flex justify-between items-center mb-4", children: [jsx("h2", { className: "text-xl font-bold text-slate-100", children: "Simple Planet Settings" }), jsx("button", { onClick: () => setShowSettings(false), className: "text-slate-300 hover:text-slate-100", "aria-label": "Close settings", children: jsxs("svg", { xmlns: "http://www.w3.org/2000/svg", width: "24", height: "24", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: "2", strokeLinecap: "round", strokeLinejoin: "round", children: [jsx("line", { x1: "18", y1: "6", x2: "6", y2: "18" }), jsx("line", { x1: "6", y1: "6", x2: "18", y2: "18" })] }) })] }), jsx(SimplePlanetSettings, { planetConfig: planetConfig, onChange: onConfigChange })] }))] }));
}
