...
import type { SurfacePointInfo } from "../planet/planet-main"
...
export function PlanetShader({ planetStats }: PlanetShaderProps) {
  ...
  useEffect(() => {
    try {
      // Ensure planetMaterial.uniforms exists
      if (!planetMaterial || !planetMaterial.uniforms) {
      ...
      if (!planetStats.landmarks || !Array.isArray(planetStats.landmarks) || planetStats.landmarks.length === 0) {
        ...
        planetMaterial.uniforms.landmarkPositions.value = new Float32Array(30)
        planetMaterial.uniforms.landmarkInfluences.value = new Float32Array(40)
        return
      }
      ...
      for (let i = 0; i < landmarksToUse.length; i++) {
        ...
        influencesArray.push(
          typeof landmark.influence_radius === "number" ? landmark.influence_radius : 0.5,
          typeof landmark.influence_strength === "number" ? landmark.influence_strength : 0.7,
          typeof landmark.influence_roughness === "number" ? landmark.influence_roughness : 0.5,
          typeValue,
        )
      }

      // Update shader uniforms with flat arrays
      planetMaterial.uniforms.landmarkPositions.value = new Float32Array(positionsArray)
      planetMaterial.uniforms.landmarkInfluences.value = new Float32Array(influencesArray)
      planetMaterial.uniforms.landmarkCount.value = positionsArray.length / 3 // Divide by 3 because each position is x,y,z
    } catch (error) {
  ...
  }, [planetStats.landmarks, planetMaterial])
  ...
  const handlePlanetClick = (event: THREE.Event) => {
    ...
    const mouse = new THREE.Vector2(
      ((event as MouseEvent).clientX / gl.domElement.clientWidth) * 2 - 1,
      -((event as MouseEvent).clientY / gl.domElement.clientHeight) * 2 + 1,
    )
  ...
  const landmarkMarkers = useMemo(() => {
    try {
      ...
      return activeLandmarks
        .map((landmark, index) => {
          if (!landmark || !landmark.coordinates) return null

          // Calculate position on planet surface with fallbacks
          const x = typeof landmark.coordinates.x === "number" ? landmark.coordinates.x : 0
          const y = typeof landmark.coordinates.y === "number" ? landmark.coordinates.y : 0
          const z = typeof landmark.coordinates.z === "number" ? landmark.coordinates.z : 0
          ...
          if (landmark.type && typeof landmark.type === "string") {
      ...
      console.error("Error rendering landmark markers:", error)
      return null
    }
