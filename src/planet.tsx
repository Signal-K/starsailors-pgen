...
} from "../lib/planet-physics.ts";
import { noise } from "../lib/noise";
import { getBiomeColors, modifyColorForSoilType } from "../lib/biome-data.ts";
...
export function Planet({ planetStats }: { planetStats: PlanetStats }) {
  ...
  // Generate clouds
  const cloudMeshes: { key: string; position: THREE.Vector3; size: number }[] = useMemo(() => {
    if (planetType !== "terrestrial" || !planetStats.cloudCount || planetStats.cloudCount <= 0) {
      return []
    }

    const cloudMeshes: { key: string; position: THREE.Vector3; size: number }[] = []
